...
class SqliteQuestionRepository extends QuestionRepository {
  @override
  Future<List<OceanQuestion>> getOceanQuestions() async {
    final db = await DatabaseHelper.instance.database;
    final rows = await db.query(AppTables.oceanQuestions, orderBy: 'id ASC');
    return rows
        .map(
          (r) => OceanQuestion(
            id: r['id'] as int,
            trait: OceanTraitX.parse(r['trait'] as String),
            positiveKeyed: (r['positive_keyed'] as int) == 1,
            statement: LocalizedString(
              id: r['statement_id'] as String,
              en: r['statement_en'] as String,
            ),
          ),
        )
        .toList();
  ...
  Future<PsychData> getPsychData() async {
    final db = await DatabaseHelper.instance.database;
    final qRows = await db.query(AppTables.psychQuestions, orderBy: 'id ASC');
